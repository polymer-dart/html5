...
class EventStreamProvider<T> {
  ...
  const EventStreamProvider(String type) : _type = type;

  Stream<T> forTarget(EventTarget tgt) => new _EventStreamProvider(tgt, _type)._controller.stream;

  call(EventTarget t) => forTarget(t);
}

EventStreamProvider<KeyboardEvent> onKeyDown = const EventStreamProvider('keydown');
...
abstract class KeyCode {
  ...
  static bool isCharacterKey(int keyCode) {
    if ((keyCode >= ZERO && keyCode <= NINE) || (keyCode >= NUM_ZERO && keyCode <= NUM_MULTIPLY) || (keyCode >= A && keyCode <= Z)) {
      return true;
    }

    // Safari sends zero key code for non-latin characters.
    /* TODO : RESTORE THIS LINES
...
bool matchesWithAncestors(HTMLElement elem, String selectors) {
  do {
...
List<Node> asList(NodeList nodeList) => new List.from(asIterable(nodeList));

Iterable<Node> asIterable(NodeList nodeList) sync* {
  for (int i = 0; i < nodeList.length; i++) yield nodeList[i];
}
